<#PSScriptInfo
.VERSION 1.0.0
.GUID 2d5b6a53-a2c2-47dc-8f04-5b6f4e2a7fcb
.TAGS Microsoft Entra, Microsoft Graph, Audit, Inactive Users, HTML
#>

<#
.SYNOPSIS
  Report-only Microsoft Entra inactive users with pretty HTML, filters, CSV export, and column toggles.

.DESCRIPTION
  Connects to Microsoft Graph with least-privilege read scopes, enumerates users and their sign-in activity,
  computes inactivity, and writes an interactive HTML report (no deletes/updates). Includes:
    - Summary cards (Total, Enabled/Disabled, Licensed, External, Never Logged In)
    - New: Interactive sign-ins breakdown (≤30d, 31–90d, >90d)
    - Search box, switches, preset inactivity filters
    - Fullscreen table modal
    - CSV export of currently visible rows/columns
    - New: Column selection checkboxes to show/hide columns

.PARAMETER OutPath
  Folder to write the HTML file.

.PARAMETER OpenBrowser
  Open the generated HTML after creation.
